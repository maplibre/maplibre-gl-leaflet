...
<script type="module">
  import * as L from 'leaflet';
  import { maplibreGL } from 'https://unpkg.com/@maplibre/maplibre-gl-leaflet@latest/dist/leaflet-maplibre-gl.mjs';
...
The UMD build (`dist/leaflet-maplibre-gl.js`) remains available for MapLibre GL JS v2-v5 and existing `L.maplibreGL(...)` integrations.

### Developing this package

The files in `dist/` are generated by `npm run build`, but they are checked in because the repository's hosted examples load them directly. After changing a file in `src/`, rebuild and commit the corresponding `dist/` changes.
