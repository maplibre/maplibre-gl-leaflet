...
<script type="module">
  import * as L from 'leaflet';
  import { maplibreGL } from 'https://unpkg.com/@maplibre/maplibre-gl-leaflet@0.2.0/dist/leaflet-maplibre-gl.mjs';
...
The files in `dist/` are generated by `npm run build` and are ignored by Git. The npm release process generates and includes them automatically. Run the build before opening debug pages that reference local artifacts.
